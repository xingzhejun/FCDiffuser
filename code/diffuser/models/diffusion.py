...
def batch_dft(input_batch, clip_mode):
    dft_result = torch.fft.fft(input_batch, dim=1)
    mask = torch.zeros_like(dft_result)
    mask[:, :clip_mode, :] = 1  
    mask[:, -clip_mode:, :] = 1  
    filtered_freq = dft_result * mask

    filtered_batch = torch.fft.ifft(filtered_freq, dim=1).real  
...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        if_base=False, clip_mode = 10, base_stage_ema_model = None):
        super().__init__()
        self.horizon = horizon
        self.observation_dim = observation_dim
        self.action_dim = action_dim
        ...
        if if_base:
            self.FSC = FSC(input_dim=observation_dim, hidden_dim=256, output_dim=observation_dim)
        else:
            self.base_stage_ema_model = base_stage_ema_model
    ...
    def p_mean_variance(self, x, cond, t, returns=None):
        if self.returns_condition:
            ...
            if self.if_base:
                ...
                base = self.base_stage_ema_model.conditional_sample(cond, returns=returns)
                base = base.reshape(base.shape[0],-1)
                epsilon_cond = self.model(x, cond, t, returns, base=base, use_dropout=False)
                epsilon_uncond = self.model(x, cond, t, returns, base=base, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w*(epsilon_cond - epsilon_uncond)
        else:
            if self.if_base:
                epsilon = self.model(x, cond, t)
                ...
                base = self.base_stage_ema_model.conditional_sample(cond, returns=returns)
                base = base.reshape(base.shape[0],-1)
                epsilon = self.model(x, cond, t, base=base)    
    ...
    def p_losses(self, x_start, cond, t, returns=None):
        noise = torch.randn_like(x_start)
        if self.if_base:
            x_start = batch_dft(x_start, self.clip_mode)
        ...
        if self.if_base:
            ...
            x_recon = self.model(x_noisy, cond, t, returns, base=base)
