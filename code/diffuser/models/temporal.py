...
class TemporalUnet(nn.Module):
        ...
        if self.returns_condition:
            ...
            if if_base:
                ...
                self.base_mlp = nn.Sequential(
                        nn.Linear(horizon * transition_dim, dim),
                        act_fn,
                        nn.Linear(dim, dim * 4),
                        act_fn,
                        nn.Linear(dim * 4, dim),
    ...
    def forward(self, x, cond, time, returns=None, use_dropout=True, force_dropout=False, base=None):
        '''
        ...
        if self.returns_condition:
            ...
            if self.if_base:
                ...
                base = self.base_mlp(base)
                t = torch.cat([t, returns_embed, base], dim=-1)
        elif self.if_base:
            base = self.base_mlp(base)
            t = torch.cat([t, base], dim=-1)         
